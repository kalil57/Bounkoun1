...
const repoName = "Bounkoun-WritingEngine";
...
2. Or run this script locally with: GITHUB_TOKEN=your_token node push_to_github.js
  ...
  try {
    ...
    const response = await fetch("https://api.github.com/user/repos", {
      ...
      body: JSON.stringify({
        name: repoName,
        description: "Writing Engine Microservice for Bounkoun System",
        private: false,
        auto_init: false
      })
    });
  ...
  // 2. Initialize and configure Git locally inside Bounkoun-WritingEngine
  try {
    ...
    try {
      ...
      execSync('git commit -m "Initial Bounkoun Writing Engine setup"', { stdio: "inherit" });
    } catch (e) {
    ...
    console.log(`\n🎉 SUCCESS! Entire "Bounkoun-WritingEngine" service pushed to https://github.com/${username}/${repoName}`);
  } catch (error) {
